...
interface KanjiPreviewProps extends HTMLAttributes<HTMLDivElement> {
  kanji: Kanji;
  imageDisabled?: boolean;
}
const KanjiPreview: FC<KanjiPreviewProps> = ({ kanji, imageDisabled, ...divAttributes }) => (
  <div
    {...divAttributes}
    className={
  ...
  >
    <div className="relative m-2 flex w-[30%] min-w-[8rem] max-w-[10rem] flex-col items-center justify-center rounded bg-slate-200 px-3 py-1">
      ...
      <p className="text-sm text-slate-500">{kanji.Popularity && `В топе: #${kanji.Popularity}`}</p>
